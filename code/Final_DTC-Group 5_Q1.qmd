...
Independent Variables: binary variable After_covid, and categorical variable year.
...
m1 <- feols(avg_emp ~ after_covid, data=df1, se='hetero')
# add year as a categorical control
m2 <- feols(avg_emp ~ after_covid + factor(year), data=df1, se='hetero')
etable(m1, m2)
```
...
From the statistical summary, we can see that with model 1, before Covid-19, the mean of the average employment rate per month in retail industry is 0.9565, while after Covid-19, the mean of the average employment rate per month is estimated to decrease by 0.0324. And this effect is highly statistically significant even at 99.9% level. With model 2, before Covid-19, the mean of average employment rate per month is estimated to be 0.9571. After Covid-19 and compared to year 2019, in year 2020 the mean of average employment rate is estimated to decrease by 0.0587; after Covid-19 and compared to year 2019, in year 2021, the mean of average employment rate is estimated to decrease by 0.0189, and this is statistically significant at 99% confidence level; after Covid-19 and compared to year 2019, in year 2022, the mean of average employment rate is estimated to decrease by 0.0076, and this is statistically significant at 99.9% confidence level. These results align with the actual data.

## Statistics Testing

Finally, we ran an F testing to see if the coefficients of years is joinly zero, because in model 2, the coefficient of year 2020 is not statistically significant.

```{r}
wald(m2, 'year')
```

The p-value is 0.001148, therefore, we are able to reject the null hypothesis at 99% confidence level. We do need to include year as a categorical control in the regression.
